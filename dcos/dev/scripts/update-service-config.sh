...
dcos marathon app show $SERVICE_ID > /tmp/$SERVICE_ID.json
# Delete version from the app config
cat /tmp/$SERVICE_ID.json | jq 'del(.version)' | jq  "$SERVICE_CONFIG_JQ" > /tmp/$SERVICE_ID-temp.json
dcos marathon app stop $SERVICE_ID --force
# wait until tasks are stopped
breakOnStatus '.tasks' "[]"

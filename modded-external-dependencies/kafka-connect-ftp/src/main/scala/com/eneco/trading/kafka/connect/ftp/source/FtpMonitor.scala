...
class FtpMonitor(settings:FtpMonitorSettings, fileConverter: FileConverter) extends StrictLogging {
  ...
  def fetch(file: AbsoluteFtpFile, prevFetch: Option[FileMetaData]): Option[(Option[FileMetaData], FetchedFile)] = {
	...
	val fileRetrieve = Try(ftp.retrieveFile(file.path, baos))
	fileRetrieve match {
      case Success(data) =>
        if(data) {
      ...
      case Failure(e) =>
        logger.error(s"Error in fetching file ${file.path}: ${ftp.getReplyString}")
        e match {
   			case e1: IOException => throw new RetriableException("Retrying on IOException")
   			case _ => throw e
		}      	
    }
    ...
    if (!ftp.isConnected) {
      ...
      settings.port match {
        case Some(explicitPort) => 
          ftpConnector=Try(ftp.connect(settings.host, explicitPort))
        case None => 
          ftpConnector=Try(ftp.connect(settings.host))
      }
      ftpConnector match {
        case Success(data) =>
        ...
        case Failure(e) =>
          logger.error("Error occurred in connect")
          return Failure(new Exception("cannot connect to ftp because of some unreported error"))
      }

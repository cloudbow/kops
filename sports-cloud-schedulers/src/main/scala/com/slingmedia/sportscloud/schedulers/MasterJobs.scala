...
object KafkaConnectContentMatchJob  {
  def main(args: Array[String]) {
    new KafkaConnectContentMatchJob().execute()
  }
}
...
class KafkaConnectContentMatchJob {
  ...
  def execute() {
    ...
    Seq("/deploy-scheduled-jobs/scripts/kafka-connect/launch_kafka_connect_jobs.sh",
        ...
        "/deploy-scheduled-jobs/scripts/kafka-connect/worker-config/ftp-connect-content-match.properties",
...
object KafkaConnectMetaBatchJob  {
  def main(args: Array[String]) {
    new KafkaConnectMetaBatchJob().execute()
  }
}
...
class KafkaConnectMetaBatchJob {
  private val log = LoggerFactory.getLogger("KafkaConnectMetaBatchJob")
    ...
    "/deploy-scheduled-jobs/scripts/kafka-connect/worker-config/ftp-meta-batch.properties",
...
object KafkaConnectLiveInfoJob  {
  def main(args: Array[String]) {
    new KafkaConnectLiveInfoJob().execute()
  }
}
...
class KafkaConnectLiveInfoJob {
  private val log = LoggerFactory.getLogger("KafkaConnectLiveInfoJob")
    ...
    "/deploy-scheduled-jobs/scripts/kafka-connect/worker-config/ftp-live-scores.properties",
...
class DownloadSchedulesJob {
  private val log = LoggerFactory.getLogger("DownloadSchedulesJob")
    ...
    log.trace("Executing task : DownloadSchedulesJob")
		...
		s"curl http://artifact-server.marathon.l4lb.thisdcos.directory:9082/artifacts/slingtv/summary.json" #> new File("/tmp/summary.json") ! ;
        ...
        var lower = 1
				...
				"curl --upload-file /tmp/schedules_plus_3 http://artifact-server.marathon.l4lb.thisdcos.directory:9082/artifacts/slingtv/schedules_plus_3" !
...
class DownloadThuuzJob  {
  private val log = LoggerFactory.getLogger("ThuuzJob")
    ...
    log.trace("Executing task : ThuuzJob")	  
		...
		"curl --upload-file /tmp/thuuz.json http://artifact-server.marathon.l4lb.thisdcos.directory:9082/artifacts/slingtv/thuuz.json" !
  }
}
...
class  DownloadSummaryJob {
  ...
  def execute() {
    log.trace("Executing task : DownloadSummaryJob")
		...
		"curl --upload-file /tmp/summary.json http://artifact-server.marathon.l4lb.thisdcos.directory:9082/artifacts/slingtv/summary.json" !
	}

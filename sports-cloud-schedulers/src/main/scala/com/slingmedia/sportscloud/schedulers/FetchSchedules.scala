...
object SportsCloudSchedulers {
  ...
  def main(args: Array[String]) {
    ...
    Holder.log.debug(s"Args is $args")
    ...
    args(0) match {
      ...
      case "ncaaf" =>
        ...
        val contentMatchJob: ScheduledJob = new ScheduledJob("contentMatchJobSchedule1", "sportscloud-batch-schedules", classOf[ContentMatchJob].asInstanceOf[Class[Any]], "0 */10 7 ? * *", ScheduleType.CRON_MISFIRE_NOW)
        allBatchJobs = contentMatchJob :: allBatchJobs
        Holder.log.debug("Adding contentMatchJob")
        // these are to elastic search
        ...
        // these are kafka connect jobs

        val liveStreamJob: ScheduledJob = new ScheduledJob("liveStreamJob", "sportscloud-restartable-schedules", classOf[NcaafLiveStreamJob].asInstanceOf[Class[Any]], "0 * * ? * *", ScheduleType.CRON_MISFIRE_DO_NOTHING)
        allRestartableJobs = liveStreamJob :: allRestartableJobs
        Holder.log.debug("Adding liveStreamJob")

        val kakfkConnCM: ScheduledJob = new ScheduledJob("kafkaConnContentMatch", "sportscloud-restartable-schedules", classOf[KafkaConnectContentMatchJobNcaaf].asInstanceOf[Class[Any]], "0 * * ? * *", ScheduleType.CRON_MISFIRE_DO_NOTHING)
        allRestartableJobs = kakfkConnCM :: allRestartableJobs
        Holder.log.debug("Adding kafkaConnContentMatch")

        val kakfkConnLI: ScheduledJob = new ScheduledJob("kafkaConnLiveInfo", "sportscloud-restartable-schedules", classOf[KafkaConnectLiveInfoJobNcaaf].asInstanceOf[Class[Any]], "0 * * ? * *", ScheduleType.CRON_MISFIRE_DO_NOTHING)
        allRestartableJobs = kakfkConnLI :: allRestartableJobs
        Holder.log.debug("Adding kafkaConnLiveInfo")

        val kakfkConnMB: ScheduledJob = new ScheduledJob("kafkaConnMetaBatch", "sportscloud-restartable-schedules", classOf[KafkaConnectMetaBatchJobNcaaf].asInstanceOf[Class[Any]], "0 * * ? * *", ScheduleType.CRON_MISFIRE_DO_NOTHING)
...
class KafkaConnectContentMatchJob extends Job {
  ...
  override def execute(context: JobExecutionContext) {
    ...
    Seq("/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/launch_kafka_connect_jobs.sh",
      ...
      "1800000",
      "/project/sports-cloud-parsers/src/main/resources/kafka-workers/cs-content-match.properties",
      "/project/sports-cloud-parsers/src/main/resources/kafka-connector-tasks/ftp-connect-content-match.json",
      "/var/log/sports-cloud-kafka-jobs/cs-content-match-kafka-connect.log",
      "ftp-content-match") !
  }
}

class KafkaConnectContentMatchJobNcaaf extends Job {
  private val log = LoggerFactory.getLogger("KafkaConnectContentMatchJob")
  override def execute(context: JobExecutionContext) {
    log.trace("Executing task : KafkaConnectContentMatchJob")
    Seq("/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/launch_kafka_connect_jobs.sh",
      ...
      "1800000",
      "/project/sports-cloud-parsers/src/main/resources/kafka-workers/cs-content-match.properties",
      "/project/sports-cloud-parsers/src/main/resources/kafka-connector-tasks/ftp-connect-content-match-ncaaf.json",
      "/var/log/sports-cloud-kafka-jobs/cs-content-match-kafka-connect.log",
...
class KafkaConnectMetaBatchJob extends Job {
  ...
  override def execute(context: JobExecutionContext) {
    ...
    Seq("/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/launch_kafka_connect_jobs.sh",
      ...
      System.getProperty("zkHost"),
      "1800000",
      "/project/sports-cloud-parsers/src/main/resources/kafka-workers/cs-meta-batch.properties",
      "/project/sports-cloud-parsers/src/main/resources/kafka-connector-tasks/ftp-meta-batch.json",
...
class KafkaConnectMetaBatchJobNcaaf extends Job {
  private val log = LoggerFactory.getLogger("KafkaConnectMetaBatchJobNcaaf")
  override def execute(context: JobExecutionContext) {
    log.trace("Executing task : KafkaConnectMetaBatchJobNcaaf")
    Seq("/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/launch_kafka_connect_jobs.sh",
      "meta_batch",
      "5",
      "15",
      ...
      "1800000",
      "/project/sports-cloud-parsers/src/main/resources/kafka-workers/cs-meta-batch.properties",
      "/project/sports-cloud-parsers/src/main/resources/kafka-connector-tasks/ftp-meta-batch-ncaaf.json",
      "/var/log/sports-cloud-kafka-jobs/cs-meta-batch-kafka-connect.log",
...
class KafkaConnectLiveInfoJob extends Job {
  ...
  override def execute(context: JobExecutionContext) {
    ...
    Seq("/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/launch_kafka_connect_jobs.sh",
      ...
      System.getProperty("zkHost"),
      "1800000",
      "/project/sports-cloud-parsers/src/main/resources/kafka-workers/cs-live-info.properties",
      "/project/sports-cloud-parsers/src/main/resources/kafka-connector-tasks/ftp-live-scores.json",
      "/var/log/sports-cloud-kafka-jobs/cs-live-info-kafka-connect.log",
      "ftp-live-scores") !
  }
}

class KafkaConnectLiveInfoJobNcaaf extends Job {
  private val log = LoggerFactory.getLogger("KafkaConnectLiveInfoJobNcaaf")
  override def execute(context: JobExecutionContext) {
    log.trace("Executing task : KafkaConnectLiveInfoJob")
    Seq("/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/launch_kafka_connect_jobs.sh",
      "live_info",
      ...
      "1800000",
      "/project/sports-cloud-parsers/src/main/resources/kafka-workers/cs-live-info.properties",
      "/project/sports-cloud-parsers/src/main/resources/kafka-connector-tasks/ftp-live-scores-ncaaf.json",
      "/var/log/sports-cloud-kafka-jobs/cs-live-info-kafka-connect.log",
...
class NcaafLiveStreamJob extends SparkSubmitJob {
  private val log = LoggerFactory.getLogger("NcaafLiveStreamJob")

  override def execute(context: JobExecutionContext) {
    //This needs to be a long running job and hence for reliability we use a shell script
    Seq("/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/launch_ncaaf_live_info_streaming.sh",
      "14",
      "23",
      "0",
      "6",

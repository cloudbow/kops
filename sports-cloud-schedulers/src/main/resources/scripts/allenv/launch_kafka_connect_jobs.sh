...
function restartOnError() {
	INIT_STATUS=`getValForProp "http://localhost:8083/connectors/$connectId/tasks" ".error_code"`
	if [ "$INIT_STATUS" == "404" ]
    then
    	echo "The kafka job $topicName is not running."
    	/project/sports-cloud-schedulers/src/main/resources/scripts/allenv/kafka-connect-job.sh $connectConfig
    else
       echo "There is a status for this job"
    fi
    STATUS=`getValForProp "http://localhost:8083/connectors/$connectId/status" ".connector.state"`
...
then
   ...
   then
      ...
      then
         ...
         curl -X DELETE http://localhost:8083/connectors/$connectId
      fi
   fi
fi
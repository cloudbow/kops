...
/**
  * Handler for converting NBA Box Score source data to Player stats record
  */
class PlayerGameStatsNbaSourceConverter extends SourceRecordConverter with ConverterBase {
  ...
  override def convert(in: SourceRecord): java.util.List[SourceRecord] = {
    ...
    generatePlayerGameStatsData(in, LeagueEnum.NBA)
  }

...
class NcaafPlayerStatsParser extends ParsedItem {
  private val log = LoggerFactory.getLogger("NcaafPlayerStatsParser")
  ...
  override def generateRows(data: Elem, in: SourceRecord,xmlRoot:NodeSeq): java.util.List[SourceRecord] = {
    ...
    val rows = xmlRoot.map { rowData =>
      ...
      val message = NcaafPlayerStats(playerExternalId, playerWins, playerLosses)
  ...
  case class NcaafPlayerStats(playerExternalId: String, playerWins: Int, playerLosses: Int) {
